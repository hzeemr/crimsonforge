...
static void do_character_switch(int charIndex) {
    ...
    ulog("[SWITCH] Switching to index %d (charData=0x%X, manager=0x%p)",
         charIndex, charData, manager);

    /* Call DoSwitch — this is the mercenary path but may work for players too */
    __try {
        g_doSwitch(manager, charData);
        ulog("[SWITCH] DoSwitch returned successfully");
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        ulog("[SWITCH] DoSwitch CRASHED (exception 0x%X)", GetExceptionCode());

        /* Try alternative: call with type flag */
        uint32_t charDataAlt = 0x20000000 | charIndex;
        ulog("[SWITCH] Retrying with charData=0x%X", charDataAlt);
        __try {
            g_doSwitch(manager, charDataAlt);
            ulog("[SWITCH] DoSwitch alt returned successfully");
        } __except(EXCEPTION_EXECUTE_HANDLER) {
            ulog("[SWITCH] DoSwitch alt also CRASHED");
        }
    }
}
