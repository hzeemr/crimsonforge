...
$srcFiles = "`"$workDir\crimson_live_tracer.c`" `"$hookSrc`""
foreach ($s in $mhSources) { $srcFiles += " `"$s`"" }

$cmd = @(
    "call `"$vcvars`" >nul",
    "cd /d `"$workDir`"",
    "cl /nologo /W3 /O2 /LD /D_CRT_SECURE_NO_WARNINGS /DCRIMSON_USE_MINHOOK=1 /I`"$(Join-Path $mhRoot 'include')`" /I`"$(Join-Path $mhRoot 'src')`" /I`"$workDir`" $srcFiles /link /MACHINE:X64 /OUT:CrimsonForgeLiveTracer.dll kernel32.lib user32.lib"
) -join ' && '
...
try {
    ...
    Write-Host "  CrimsonForgeLiveTracer.asi"
} finally {
    Pop-Location
}
